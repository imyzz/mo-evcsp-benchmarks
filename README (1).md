...
All figures are generated by `Part2_Analysis.ipynb`. Run the notebook after `Part1_Generation.ipynb` to reproduce them.

---

### Figure 1 — EDA: Key variable distributions

![fig1](figures/fig1_eda_distributions.png)

**What it shows:** Histograms with KDE overlay (top row) and boxplots (bottom row) for the three session attributes extracted from the real dataset after cleaning.
...
### Figure 2 — Hourly arrival profile with TOU zones

![fig2](figures/fig2_hourly_arrival_tou.png)

**What it shows:** Half-hourly arrival density overlaid with the three TOU price zones of the Jiaxing Nanhu district (Zhang et al., 2025, Table 5).

**How to read it:**
- 🔴 **Sharp zone** (19:00–21:00, 1.2064 ¥/kWh) — high tariff coincides with the evening arrival peak → shifting load away from this zone is the main cost-reduction lever in the scheduling problem
- 🟡 **Peak zone** (08:00–11:00, 13:00–19:00, 21:00–22:00, 0.9014 ¥/kWh)
- 🟢 **Off-peak zone** (00:00–08:00, 11:00–13:00, 22:00–24:00, 0.3784 ¥/kWh) — optimal charging window

**Relevance:** this figure motivates the cost minimisation objective in the bi-objective scheduling problem.
...
**What it shows:** Scatter plots with regression lines and Pearson/Spearman correlation coefficients (with 95% CI via Fisher z-transform) between all pairs of the three key variables, computed on a 5,000-session subsample.
...
### Figure 4 — KS goodness-of-fit heatmap

![fig4](figures/fig4_ks_goodness_of_fit.png)

**What it shows:** KS statistic D and associated p-value for four parametric families fitted to each of the three key variables. Red = large D (poor fit), green = small D (better fit).

**How to read it:** Every cell corresponds to a rejected null hypothesis (p < 0.001). No parametric family provides an acceptable fit to any variable. This is the **formal statistical justification** for using the non-parametric hybrid approach rather than a standard distributional model such as the exponential-GMM combination of Lahariya et al. (2020).

---

### Figure 5 — EM-fitted GMM components vs empirical KDE

![fig5](figures/fig5_gmm_fitted.png)

**What it shows:** For each variable, individual GMM component densities (coloured, shaded areas), their weighted mixture (black solid line), and the empirical KDE (black dashed line).
...
**Key message:** all parameters (weights, means, standard deviations) are **entirely data-driven** — no value is set manually.

---

### Figure 6 — Validation: real vs synthetic distributions

![fig6](figures/fig6_validation_distributions.png)

**What it shows:** Overlapping histograms and KDE curves for real data (blue) and synthetic sessions pooled from the 100-EV scenario over 10 days (red). The KS statistic and p-value quantify distributional similarity.
...
### Figure 7 — KDE/GMM ratio sensitivity

![fig7](figures/fig7_kde_ratio_sensitivity.png)

**What it shows:** Mean KS statistic (5 replications) between real and synthetic distributions for KDE ratios ranging from 50% to 100%, for each of the three key variables.
...
**What it shows:** Box plots (10 replications per group) for six instance-level metrics across all seven fleet sizes.
...
### Figure 8 — Inter-day variability
...
**What it shows:** Session timeline for the 50-EV, Day 1 scenario. Each horizontal bar = one EV session; bar length = session duration; label = required energy (kWh). The yellow band marks the sharp TOU zone (19:00–21:00).

**How to read it:** EVs whose sessions overlap the sharp TOU zone are the primary load-shifting targets for the scheduling algorithm. The diversity of arrival times, durations, and energy demands visible here reflects the multimodal distribution of the real dataset captured by the generator.

---

## TOU pricing

Jiaxing Nanhu district electricity tariffs (Zhang et al., 2025, Table 5):

| Zone | Time windows | Price (¥/kWh) |
|---|---|---|
| Sharp (尖) | 19:00–21:00 | 1.2064 |
| Peak (峰) | 08:00–11:00, 13:00–19:00, 21:00–22:00 | 0.9014 |
| Off-peak (谷) | 00:00–08:00, 11:00–13:00, 22:00–24:00 | 0.3784 |
...
# 2. Place Charging_Data.csv in the root directory

# 3. Generate instances (scenario_1.csv … scenario_70.csv)
jupyter nbconvert --to notebook --execute Part1_Generation.ipynb

# 4. Run analysis and produce all figures
jupyter nbconvert --to notebook --execute Part2_Analysis.ipynb
```

Key parameters (configurable in `Part1_Generation.ipynb`, Section 0):

| Parameter | Value | Description |
|---|---|---|
| `SEED` | 42 | NumPy random seed (`default_rng`) |
| `KDE_RATIO` | 0.70 | Fraction of sessions generated by KDE |
| `N_COMPONENTS` | 3 | Number of GMM components |
| `P_MAX_KW` | 22.0 | Charger rated power (kW) |
| `BW_H` | 0.25 | KDE bandwidth — arrival time (h) |
| `BW_DUR` | 5.0 | KDE bandwidth — session duration (min) |
| `BW_ENG` | 1.5 | KDE bandwidth — energy (kWh) |
